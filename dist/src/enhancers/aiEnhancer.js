...
const wcag_contrast_1 = __importDefault(require("wcag-contrast"));
class AIEnhancer {
    constructor() {
        ...
        elements.forEach(element => {
            if (element.style.fill?.[0]?.color && element.type === 'text') {
                ...
                try {
                    ...
                    const txtColor = (0, color_1.default)(textColor);
                    const contrastRatio = wcag_contrast_1.default.ratio(bgColor.rgb().array(), txtColor.rgb().array());
                    if (contrastRatio < this.MIN_CONTRAST_RATIO) {
                        ...
                        enhancements.push({
                            ...
                            contrastRatio: wcag_contrast_1.default.ratio(bgColor.rgb().array(), suggestedColor.rgb().array())
                        });
                    }
                }
                catch (error) {
    ...
    findBetterContrast(background, text) {
        ...
        let attempts = 0;
        let newColor = text;
        while (wcag_contrast_1.default.ratio(background.rgb().array(), newColor.rgb().array()) < this.MIN_CONTRAST_RATIO && attempts < 10) {
